...
//    NSLog(@"在这里不断的处理请求");
    ...
    for (AVAssetResourceLoadingRequest *loadingRequest in self.loadingRequests) {
        ...
        // 2. 填充数据
        NSData *data = [NSData dataWithContentsOfFile:[XMGRemoteAudioFile tmpFilePath:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data == nil) {
            data = [NSData dataWithContentsOfFile:[XMGRemoteAudioFile cacheFilePath:url] options:NSDataReadingMappedIfSafe error:nil];
        }
